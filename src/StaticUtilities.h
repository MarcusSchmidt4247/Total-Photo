...
public:
	...
	static std::vector<File> RecurseGetImages(std::filesystem::path path, SortMethod sortMethod, const std::vector<ToggledString> &fileTypes, Directory *directory); // gathers images and recurses based on directory information
	static std::vector<File> RecurseGetImages(std::filesystem::path path, SortMethod sortMethod, const std::vector<ToggledString> &fileTypes, const std::unordered_map<std::string, int> &filterItems, const bool defaultValidity); // continues from where directory info ends
